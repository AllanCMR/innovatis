...
def mostra_resultado(pontos):
    ...
    if pontos == 20:
        ...
        lblresulttext.configure(text=f"Parabéns! Você fez {pontos} pontos e acertou todas as perguntas!"
                                     ...
                                     "\nJuntos somos mais fortes no combate ao Covid-19!", fg='#008000',
        ...
        lblimagem.configure(image=img)
        lblimagem.image = img
        lblresulttext.configure(
            text=f"Você foi bem! Fez {pontos} pontos.\n Atualize-se mais, pois sua ajuda será muito importante"
...
def selecionado():
    ...
    global lblPergunta, resp1, resp2, resp3, resp4, respc
    global ques
    x = radiovar.get()

    if ques < 20:
        if x != resp[indexes[ques - 1]]:
            respc['text'] = 'Resposta anterior incorreta! \nResposta correta: \n%s ' % resposta[indexes[ques - 1]][
                resp[indexes[ques - 1]]]
            respc['foreground'] = '#cc0000'
            respc['font'] = 'Times'
            respc.pack(pady=10)
        else:
            respc['text'] = 'Resposta anterior correta!'
            respc['foreground'] = '#00b300'
            respc['font'] = 'Times'
            respc.pack(pady=10)
...
def inicia_quiz():
    ...
    respc = Label(
        root,
        text=resposta[indexes[0]][resp[indexes[0]]],
        font=("Times bold", 15),
        background="#ffffff",
        foreground="#00FF00"
    )
